...
//| Usage: Internal structure for organization CHashMapPointer<T>.          |
//+------------------------------------------------------------------+
...
//+------------------------------------------------------------------+
//| Class CHashMapPointer<TKey, TValue>.                                    |
//| Usage: Represents a collection of keys and values.               |
//+------------------------------------------------------------------+
template<typename TKey,typename TValue>
class CHashMapPointer: public IMap<TKey,TValue>
  {
protected:
   int               m_buckets[];
...
public:
                     CHashMapPointer(void);
                     CHashMapPointer(const int capacity);
                     CHashMapPointer(IEqualityComparer<TKey>*comparer);
                     CHashMapPointer(const int capacity,IEqualityComparer<TKey>*comparer);
                     CHashMapPointer(IMap<TKey,TValue>*map);
                     CHashMapPointer(IMap<TKey,TValue>*map,IEqualityComparer<TKey>*comparer);
                    ~CHashMapPointer(void);
...
//+------------------------------------------------------------------+
//| Initializes a new instance of the CHashMapPointer<TKey,TValue> class    |
//| that is empty, has the default initial capacity, and uses the    |
//| default equality comparer for the key type.                      |
//+------------------------------------------------------------------+
template<typename TKey,typename TValue> CHashMapPointer::CHashMapPointer(void): m_count(0),
   m_free_list(0),
   m_free_count(0),
   m_capacity(0)
  {
...
//| default equality comparer for the key type.                      |
//+------------------------------------------------------------------+
template<typename TKey,typename TValue> CHashMapPointer::CHashMapPointer(const int capacity): m_count(0),
   m_free_list(0),
   m_free_count(0),
   m_capacity(0)
  {
...
//+------------------------------------------------------------------+
template<typename TKey,typename TValue> CHashMapPointer::CHashMapPointer(IEqualityComparer<TKey>*comparer): m_count(0),
   m_free_list(0),
   m_free_count(0),
   m_capacity(0)
  {
...
//| specified IEqualityComparer<TKey>.                               |
//+------------------------------------------------------------------+
template<typename TKey,typename TValue> CHashMapPointer::CHashMapPointer(const int capacity,IEqualityComparer<TKey>*comparer): m_count(0),
   m_free_list(0),
   m_free_count(0),
   m_capacity(0)
  {
...
//| key type.                                                        |
//+------------------------------------------------------------------+
template<typename TKey,typename TValue> CHashMapPointer::CHashMapPointer(IMap<TKey,TValue>*map): m_count(0),
   m_free_list(0),
   m_free_count(0),
   m_capacity(0)
  {
...
//+------------------------------------------------------------------+
template<typename TKey,typename TValue> CHashMapPointer::CHashMapPointer(IMap<TKey,TValue>*map,IEqualityComparer<TKey>*comparer): m_count(0),
   m_free_list(0),
   m_free_count(0),
   m_capacity(0)
  {
...
//| Destructor.                                                      |
//+------------------------------------------------------------------+
template<typename TKey,typename TValue> CHashMapPointer::~CHashMapPointer(void)
  {
   ClearMemmory();
...
//+------------------------------------------------------------------+
template<typename TKey,typename TValue>
bool CHashMapPointer::Add(CKeyValuePair<TKey,TValue>*pair)
  {
//--- check pair
...
//+------------------------------------------------------------------+
template<typename TKey,typename TValue>
bool CHashMapPointer::Add(TKey key,TValue value)
  {
...
//+------------------------------------------------------------------+
template<typename TKey,typename TValue>
bool CHashMapPointer::Contains(CKeyValuePair<TKey,TValue>*item)
  {
//--- check pair
...
//+------------------------------------------------------------------+
template<typename TKey,typename TValue>
bool CHashMapPointer::Contains(TKey key,TValue value)
  {
//--- find pair with specified key
   int i=FindEntry(key);
//--- create default equality value comparer
...
//+------------------------------------------------------------------+
template<typename TKey,typename TValue>
bool CHashMapPointer::ContainsKey(TKey key)
  {
...
//+------------------------------------------------------------------+
template<typename TKey,typename TValue>
bool CHashMapPointer::ContainsValue(TValue value)
  {
//--- create default equality value comparer
...
//+------------------------------------------------------------------+
template<typename TKey,typename TValue>
int CHashMapPointer::CopyTo(CKeyValuePair<TKey,TValue>*&dst_array[],const int dst_start=0)
  {
//--- resize array
...
//+------------------------------------------------------------------+
template<typename TKey,typename TValue>
int CHashMapPointer::CopyTo(TKey &dst_keys[],TValue &dst_values[],const int dst_start=0)
  {
...
//+------------------------------------------------------------------+
template<typename TKey,typename TValue>
void CHashMapPointer::Clear(void)
  {
//--- check count
   if(m_count>0)
     {
...
//+------------------------------------------------------------------+
template<typename TKey,typename TValue>
void CHashMapPointer::ClearMemmory(void)
  {
//--- check count
   if(m_count>0)
     {
         ...
         if(m_entries[i].hash_code>=0)
           {
            ...
            //dst_values[dst_start+index]=m_entries[i].value;
            //index++;
            if(CheckPointer(m_entries[i].value) == POINTER_DYNAMIC)
...
//+------------------------------------------------------------------+
template<typename TKey,typename TValue>
bool CHashMapPointer::Remove(CKeyValuePair<TKey,TValue>*item)
  {
//--- check pair
   if(CheckPointer(item)==POINTER_INVALID)
      return(false);
...
//+------------------------------------------------------------------+
template<typename TKey,typename TValue>
bool CHashMapPointer::Remove(TKey key)
  {
   if(m_capacity!=0)
     {
...
//+------------------------------------------------------------------+
template<typename TKey,typename TValue>
bool CHashMapPointer::TryGetValue(TKey key,TValue &value)
  {
//--- find pair with specified key
   int i=FindEntry(key);
//--- check index
...
//+------------------------------------------------------------------+
template<typename TKey,typename TValue>
bool CHashMapPointer::TrySetValue(TKey key,TValue value)
  {
...
//+------------------------------------------------------------------+
template<typename TKey,typename TValue>
void CHashMapPointer::Initialize(const int capacity)
  {
...
//+------------------------------------------------------------------+
template<typename TKey,typename TValue>
bool CHashMapPointer::Resize(const int new_size)
  {
//--- resize buckets
...
//+------------------------------------------------------------------+
template<typename TKey,typename TValue>
int CHashMapPointer::FindEntry(TKey key)
  {
   if(m_capacity!=NULL)
     {
...
//+------------------------------------------------------------------+
template<typename TKey,typename TValue>
bool CHashMapPointer::Insert(TKey key,TValue value,const bool add)
  {
   if(m_capacity==0)
      Initialize(0);
//--- get hash code from key
...
template<typename TKey,typename TValue>
static int CHashMapPointer::m_collision_threshold=8;
//+------------------------------------------------------------------+
//+------------------------------------------------------------------+
